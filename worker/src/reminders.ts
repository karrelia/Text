 ...
 *
 * Крім наказового способу — форми першої особи множини: жива мова частіше
 * каже «давай перенесемо нагадування», ніж «перенеси нагадування». Перша
 * версія знала тільки наказ і на «давай перенесемо» мовчала.
 */
...
/**
 * Інфінітиви небезпечні самі по собі: «нагадай прибрати сміття» — це нове
 * нагадування, а не видалення. Тож вони рахуються лише після слова, яке
 * робить із них прохання до бота: «треба прибрати нагадування…».
 */
const MANAGE_INFINITIVES = new Set([
  "прибрати",
  "забрати",
  "убрати",
  "видалити",
...
/**
 * Будь-яка ознака створення переважує керування. «Постав нагадування
 * прибрати сміття» містить «прибрати», але це нове нагадування, і
 * знищити через нього старе було б найгіршим із можливих непорозумінь.
 */
const CREATE_MARKERS = new Set([
  "поставити",
  "додати",
  "створити",
  "записати",
...
export function manageIntent(text: string): ManageIntent {
  ...
  if (
    words.some(
      (word) =>
        STRONG_WORDS.has(word) || IMPERATIVES.has(word) || CREATE_MARKERS.has(word),
    )
  ) {
    return "none";
  }
  ...
  // Інфінітив рахуємо лише тоді, коли перед ним слово, що звертає його до
  // бота: «треба прибрати нагадування», а не «прибрати сміття».
  const asked = words.some(
    (word, index) =>
      MANAGE_INFINITIVES.has(word) &&
      words.slice(Math.max(0, index - 3), index).some((prev) => HORTATIVES.has(prev)),
  );
  if (asked) return "change";
